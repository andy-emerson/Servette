...
**Placeholder and self-test pages.** Two pages ship inside the package: the placeholder below, and `servette/selftest.html` — the connection self-test, read once at import and served at the reserved `/selftest/` path wherever the operator's content doesn't shadow it, behind the site's own auth. Execution stays in the visitor's browser (only an outside client sees the browser-trusted chain, the real network path, the provider firewall); because the page ships with the server, its checks cannot drift from the features they check.

**Placeholder page (`_seed_placeholder` — code in Shell).** Setup must never finish with nothing to serve: an empty `serve_dir` would 404 on the operator's own domain with no way to tell the server from the content. The page that fills that moment is embedded in the module itself ([#70](https://github.com/andy-emerson/Servette/issues/70)) — a small, script-free "under construction" page in Servette's theme — so seeding involves no network at all. `_seed_placeholder` writes the page as the site's `index.html`: directory requests already fall back to `index.html`, so it appears at the site root with no change to path resolution, and the operator's own page later replaces it with no cleanup step. The file carries a `servette:demo` marker comment — the string keeps its historical name so pages seeded by fetch-era releases stay recognizable — and that marker is the whole ownership protocol: a marked `index.html` is Servette's placeholder, refreshed by the startup pass when the embedded copy changes; an unmarked one is the operator's and is never touched; deleting the marker adopts the page permanently. The rule is visible in the file itself, not hidden in state. The full connection self-test reaches an operator's site through the publish channel: the publish tool's starter bundle and `/selftest/` injection ([#42](https://github.com/andy-emerson/Servette/issues/42)) put it on the operator's own origin, one publish away.
...
One principle shapes the layout: **one subdomain ↔ one self-contained `site/<name>/` directory, and every page is a directory holding an `index.html`.** What a page is for is said by its directory, never its file name, so a file's name can't diverge from how it is served. The server reserves three names, and only three: `404.html` at a serve_dir root (the custom error page), `/.well-known/servette` (version discovery), and `/selftest/` (the embedded self-test — shadowed by operator content that simply exists there). Self-containment is load-bearing, not stylistic: path resolution confines each site to its own `serve_dir` (symlinks out are refused), so a subdirectory served as its own site cannot reach a sibling's assets. Because the subdirectories sit inside the apex `serve_dir`, each property is also reachable as a path — `servette.org/src/` works even where `src.servette.org` isn't configured.
