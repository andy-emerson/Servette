...
- **Tests green.** The `Tests` workflow passes on the supported Python versions (CI runs 3.11 and 3.14, plus Debian 12's own 3.11). It is four checks in one gate: `tests/test.py`, `build.py --check` (the module matches `src/`), `build.py --check-counts` (the line-count figures README states match `src/`), and `build.py --check-docs` (every path, identifier, flag, command and link the documents name exists). A behavior change ships with the test that would have caught its absence; a claim that is a number ships with the run that keeps it true.
