  ...
  function buildSiteCard(siteData, idx) {
    ...
    const bodyHtml = inactive
      ...
      : (`<div class="rows info"></div>` +
         ...
         // ── Whether visitors' browsers cache. Applied on change — both
         ...
         `<p class="cfg-hint">Enabled: browsers keep copies but re-check ` +
         `every visit, so new content is instant. Disabled: nothing is ` +
         `stored on visitors' machines. Switching public/private resets ` +
         `this to its default (public on, private off).</p>` +
    ...
    async function loadVersions() {
        ...
        try {
          ...
          // grid. Each option carries date, size, and the live marker;
          // Restore acts on the selection, dim while the live version
          // is the one selected.
          list.innerHTML = rows.length < 2 ? '' :
            ...
            rows.map((r) =>
              ...
              escapeHtml(`${when(r.published)} — ${r.files} file` +
                         `${r.files === 1 ? '' : 's'}, ${fmtSize(r.bytes)}` +
                         (r.live ? ' · live' : '')) +
              `</option>`).join('') +
            `</select>` +
            `<button class="action tiny ver-restore" type="button">` +
            `Restore</button></span></span></div>` +
            `<p class="cfg-hint">The ${v.keep} most recent are kept; ` +
            `restoring does not consume one — you can restore back.</p>`;
