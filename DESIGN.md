...
**Cert watchdog (`_cert_watchdog`).** A daemon thread polling every 60s: for a configured domain, renews when the cert expires in < 30 days (at most once per hour on failure); for self-signed certs, detects external file changes by mtime and reloads. Each pass runs in `_cert_watchdog_tick()`, which contains its own exceptions — a failed pass logs and retries rather than killing the thread (a dead watchdog would silently end renewals). Applying a new cert under `--serve` works by stopping the server and letting `_watch_server` exit non-zero, so systemd relaunches with the new cert — the sandboxed unit user cannot `systemctl restart` itself; the interactive shell (root) restarts the unit directly, and session mode does an in-process stop/start gated by `_wait_for_port_free()`.
...
Three independent triggers can call `_check_for_content_update()` concurrently — poke's background thread, the fallback poll, and the interactive `pull` command — plus `cmd_restore_site()` touching the same `serve_dir`/`serve_dir.bak` paths. `_publish_lock` (held for fetch through swap) serializes all of them, so two overlapping publishes can never interleave their renames; `cmd_restore_site()` takes the same lock only around its own rename, re-checking the backup still exists first (a concurrent publish can consume it while an operator is sitting at the confirmation prompt). Poke's own cooldown check-then-set is separately locked (`_poke_cooldown_lock`) — poke needs no auth, so concurrent requests racing an unlocked check could each read a stale timestamp and each spawn a check, defeating the cooldown.
