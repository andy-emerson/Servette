...
What travelled with it: the front page, the source viewer (`servette.org/src/`, a read-only literate view of this repository's `src/*.md`, fetched from GitHub at render time), the publish tool (`servette.org/pub/`), and the viewer's end-to-end harness. What stays here: the program, and `servette/selftest.html` — the diagnostic page, which ships *inside* the package because every install serves it.
