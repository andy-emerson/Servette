...
`site/demo/index.html` is the website's live demo — the connection self-test at demo.servette.org, which the home page links as "Open the live demo." It is the website's page alone: a fresh install seeds the placeholder embedded in the module ([#70](https://github.com/andy-emerson/Servette/issues/70)), and the self-test reaches operator sites through the publish channel (`site/pub/selftest/`). Its checks and `site/pub/selftest/`'s are deliberate duplicates — no build step shares them, so a change to either page's checks belongs in both.
