...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* a.action is a link in button dress: the View-preview control must be
       an anchor (the open happens on the operator's own click, with
       rel=noopener as the ruled isolation) but reads as the third button
       of its row. aria-disabled is its disabled attribute — an anchor
       without an href goes nowhere, and the style says so. */
    ...
    a.action:hover:not([aria-disabled="true"]) { background: rgba(90,132,102,0.28); }
    button.action:disabled, a.action[aria-disabled="true"] {
      color: var(--muted);
...
<script>
  ...
  let installSource = '';     // same call — the install's PEP 610 origin URL,
                              // '' for a PyPI install; switches the row's advice
  ...
  async function checkUpgrade() {
    try {
      const u = await getJSON('/update');
      latestVersion = u.latest || null;
      installSource = u.direct_url || '';
  ...
  function buildSiteCard(siteData, idx) {
    ...
    const bodyHtml = inactive
      ...
      : (`<div class="rows info"></div>` +
         ...
         `<div class="btn-row" style="margin-top:0.75rem">` +
           ...
           `publish, not the live site">Build preview</button>` +
           // The row's third button, dim until a preview exists to view —
           // like Save until a form can be saved. An anchor in button
           // dress, deliberately: the open must be the operator's own
           // click (a tab opened when the async staging finishes would be
           // popup-blocked), and rel=noopener is the ruled isolation — the
           // draft's tab holds no handle back to the page whose address
           // carries the passcode. Its href exists only once built.
         ...
         `<div class="preview hidden">` +
           ...
           `tunnel and not published — View preview opens it full size in ` +
           `its own tab. Links inside it work; the site's real domain, ` +
           `certificate, and headers are not part of what you are ` +
           `seeing.</p>` +
         `</div>` +
    ...
    prevBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const viewBtn = q('.preview-open');
        viewBtn.href = '/preview/' + encodeURIComponent(data.token) +
          '/' + encodeURIComponent(siteIndex()) + '/';
        viewBtn.removeAttribute('aria-disabled');
        viewBtn.title = 'The staged draft, full size in its own tab';
  ...
  function renderServer() {
    ...
    // The advice must be the command that actually works for THIS install:
    // on a URL install (PEP 610), 'pipx upgrade' re-resolves the recorded
    // URL forever and releases never arrive — the reinstall re-points it.
    const upgradeHint = installSource
      ? ` — this install came from <b>${escapeHtml(installSource)}</b>, not` +
        ` PyPI: <b>pipx install --force servette</b> in the terminal` +
        ` re-points it, then <b>enable</b>`
