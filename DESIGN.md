...
**The diagnostic page (`servette/selftest.html`).** One page ships inside the package, read once at import and served in two roles behind the site's own auth: at the reserved `/selftest/` path at status 200 wherever the operator's content doesn't shadow it, and as the default error page at status 404 wherever the operator has written no `404.html`. Execution stays in the visitor's browser (only an outside client sees the browser-trusted chain, the real network path, the provider firewall); because the page ships with the server, its checks cannot drift from the features they check. Read once at import is the one sharp edge: a running process keeps the copy it loaded, so a code update reaches the page only on restart, while site files are re-read per request.
...
Because this covers a site's own root while nothing is published there, setup and `add-site` no longer seed a page: they create the folder, say what will answer, and write nothing. The embedded placeholder page and the `servette:demo` ownership protocol that made seeding safe are deleted ([ruling](DECISIONS.md#the-default-error-page-diagnoses-the-placeholder-is-retired)). The accepted cost is that an unpublished root answers 404 rather than the placeholder's 200.
...
`site/` is the source of the project's own website, and — under `SERVETTE_HOME=.` — a development checkout's `serve_dir`, so a checkout serves servette.org, which is also its production deployment (the box serving servette.org is a Servette running this repository). Users never receive this folder: a user installs the package alone, and setup leaves them an empty site folder that answers with the embedded diagnostic page until they publish.

One principle shapes the layout: **one subdomain ↔ one self-contained `site/<name>/` directory, and every page is a directory holding an `index.html`.** What a page is for is said by its directory, never its file name, so a file's name can't diverge from how it is served. The server reserves three names, and only three: `404.html` at a serve_dir root (the custom error page, which also replaces the embedded diagnostic page as the default miss), `/.well-known/servette` (version discovery), and `/selftest/` (the diagnostic page — shadowed by operator content that simply exists there). Self-containment is load-bearing, not stylistic: path resolution confines each site to its own `serve_dir` (symlinks out are refused), so a subdirectory served as its own site cannot reach a sibling's assets. Because the subdirectories sit inside the apex `serve_dir`, each property is also reachable as a path — `servette.org/src/` works even where `src.servette.org` isn't configured.

`site/src/index.html` is a read-only literate view of `src/*.md`, fetched from this repository at render time — a `?ref=` parameter (charset-validated, no `..` segments, pinned to this repo) selects the branch or tag, so the page always shows real authored bytes, never a copy that can drift. It was grown from the notebook interface Servette was written in ([andy-emerson/notebook](https://github.com/andy-emerson/notebook)) by removing everything that edits; what remains is the reading half of the real tool — file navigation and search, a rendered preview, and a call map of the open file's functions. An in-browser AI reading assistant (WebLLM over WebGPU) was tried and removed — it read the code poorly for its cost in page weight, and it was the one surface where the project's name attached to a first-person persona rather than a program. The page persists nothing about a visitor except that the welcome dialog was shown; every other piece of state — open file, pane layout, theme, preferences — resets on every visit.

`site/pub/index.html` is the client-side publish tool ([#42](https://github.com/andy-emerson/Servette/issues/42)): it generates or loads an Ed25519 key, builds and signs a content bundle from a folder the operator picks — or from the demo starter site when they have none — and hands back the `.tar.gz` + `.sig` pair with the `publish_key` hex for `config > publish` — the operator hosts the pair anywhere and runs `pull`, so the trigger stays with the person who owns the machine. Nothing is bundled for verification: every install serves the embedded self-test at the reserved `/selftest/` path ([ruling](DECISIONS.md#the-self-test-is-server-delivered-client-executed)), so the page that checks a deployment ships with the server it checks, and the publish tool makes no network request at all. The whole pipeline is browser primitives (WebCrypto Ed25519, `CompressionStream`, a hand-rolled ustar writer emitting only the entry types `_extract_bundle` accepts), because the page's custody constraints — dependency-free, signing-only, nothing extractable stored — are load-bearing ([ruling](DECISIONS.md#the-publish-tools-custody-constraints)); the page's own header comment carries them in full. Linked from the home-page footer as the path form `pub/` — alive the moment the site deploys, while the subdomain waits on DNS.
