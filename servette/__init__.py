...
def _systemd_unit(python_path, package_dir):
    ...
    The package directory is pinned read-only on top: strict mode already covers
    it wherever pip puts it, and the pin says so explicitly — a compromised
    serving process cannot patch the program systemd will restart it into.

    No PYTHONPATH: the unit resolves `-m servette` from the interpreter's own
    site-packages and nowhere else. A unit that named a package parent could
    start a service from a directory pip does not own — a second installation
    path — and would put that entry ahead of the stdlib, widening what a write
    there could shadow. `_write_unit_files` refuses to write a unit at all
    unless the package is pip-installed, so this cannot be reached by accident.
...
def _write_unit_files():
    ...
    # One installation path, enforced where it would otherwise fork. A unit
    # written against a package outside site-packages would make "clone it and
    # enable the service" work, which is a second way to install Servette and
    # therefore a second thing to support, document, and get wrong. pip owns
    # installation; this refuses rather than quietly writing a unit that
    # depends on a directory pip does not manage.
    pkg_parent = os.path.basename(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    if pkg_parent not in ("site-packages", "dist-packages"):
        print("  Error: Servette is not pip-installed here — the package sits in")
        print(f"  {pkg_parent!r}, not site-packages. Installing as a service needs")
        print("  a pip install: pip install servette")
        raise ValueError("service install requires a pip-installed package")
