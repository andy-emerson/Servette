  ...
  function buildSiteCard(siteData, idx) {
    ...
    const bodyHtml = inactive
      ...
      : (`<div class="rows info"></div>` +
         ...
         // ── Whether visitors' browsers cache. A literal toggle, like
         // Access (ruled: two states get a switch, not a menu). Applied
         // on change — both states are safe and reversible, so a Save
         // button would be ceremony; a refused write flips the knob back.
         ...
         `<p class="cfg-hint">On: browsers keep copies and re-check them ` +
         `every visit, so a visitor is never shown stale content. Off: ` +
         `nothing is stored on visitors' machines. Switching public/private ` +
         `resets this to its default (public on, private off).</p>` +
    ...
    {
      const sw = q('.cache-switch'), state = q('.cache-state');
      const show = () => { state.textContent = sw.checked ? 'on' : 'off'; };
      sw.checked = (siteData.cache || 'yes') === 'yes';
      show();
      sw.addEventListener('change', async () => {
        show();
        const ok = await saveSettings({ cache: sw.checked ? 'yes' : 'no' },
                                      cardIndex(card), badge, errEl);
        if (!ok) { sw.checked = !sw.checked; show(); }
      });
    }
