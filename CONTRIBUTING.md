...
**Use the agent well.** Agents do their best work on bounded, well-described tasks, and this repository is structured to provide them — point the agent at [`DESIGN.md`](DESIGN.md) for how it's built and [`DESIGN.md`](DESIGN.md) for the scope and the change loop. Treat "tests pass" as the start of your review, not the end: a test can encode the same misunderstanding as the code it checks.
