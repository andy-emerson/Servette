...
`site/publish/index.html` is the client-side publish tool ([#42](https://github.com/andy-emerson/Servette/issues/42)): it generates or loads an Ed25519 key, builds and signs a content bundle from a folder the operator picks — or from the demo starter site when they have none — and hands back the `.tar.gz` + `.sig` pair with the `publish_key` hex for `config > publish` — the operator hosts the pair anywhere and runs `pull`, so the trigger stays with the person who owns the machine. Bundles carry the connection self-test as content by default (`site/publish/selftest/`, injected at `/selftest/` unless the folder brings its own): a cross-origin probe from the website could never read what it tests, but a page pulled onto the operator's own origin runs the demo page's full checks after every publish. The whole pipeline is browser primitives (WebCrypto Ed25519, `CompressionStream`, a hand-rolled ustar writer emitting only the entry types `_extract_bundle` accepts), because the page's constraints — recorded in its README — are dependency-free (it handles a private key, so no third-party script), signing-only (every capability must reduce to "produce a signed artifact the operator chooses to pull"), and nothing extractable stored (the key file is supplied per session and held as a non-extractable `CryptoKey`; the operator can opt in to remembering it in IndexedDB, where the stored handle can sign but never be read out — the key file stays the only durable copy). Not linked from the home page yet.
