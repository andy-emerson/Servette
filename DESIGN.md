...
Linux with systemd is the production target: the service user, ambient-capability port binding, sandboxing, restart-on-death, the network watchdog, and the journal are all systemd facilities, and CI runs the suite on Ubuntu and Debian 12. **macOS runs in session mode** ([#64](https://github.com/andy-emerson/Servette/issues/64)): serving, certificates (self-signed and Let's Encrypt), the shell, and self-update all work under `sudo python3 servette.py`, while service installation stays Linux-only — the `_IS_MACOS` flag marks every seam, and the suite exercises both sides of each on any host. A launchd port was considered and not taken: launchd has no equivalent of the ambient-capability trick or the systemd sandbox, so a macOS *service* would run with a weaker security posture than the Linux one — session mode keeps the posture honest. **Windows is a recorded non-goal** (ruled in #64): no systemd, no POSIX venv/exec model, and a refactor far larger than the audience it serves. Reopen trigger: credible demand from operators who cannot run Linux or macOS.
...
`site/demo/index.html` is the website's live demo — the connection self-test at demo.servette.org, which the home page links as "Open the live demo." It is no longer a release asset: since [#70](https://github.com/andy-emerson/Servette/issues/70) an empty site is seeded with the placeholder embedded in `servette.py`, and the self-test reaches operator sites through the publish channel (`site/pub/selftest/`) instead. Its checks and `site/pub/selftest/`'s are deliberate duplicates — no build step shares them, so a change to either page's checks belongs in both.
...
A release carries two assets — `servette.py` and `servette.py.sig` — and is published only after verification, so the update channel can never serve an unverified artifact:

1. Bump `__version__` in `servette.py` via its own pull request, and merge it — the only change that ever touches the version.
2. Sign the asset: `.servette-env/bin/python3 src/release.py` (key defaults to `./servette_signing.pem`). It refuses a drifted `servette.py` and a key that doesn't match the pinned `_SIGNING_PUBLIC_KEY`, then stages both files in the gitignored `dist/`.
3. **Draft** a GitHub release tagged with the version, the tag pointing at the merged bump commit, and attach the two files from `dist/`. A draft is invisible to users and to `cmd_update`.
