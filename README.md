...
| Readable source | ~4,400 lines | ~4,600 lines | binary | binary |
| Actively maintained | ✓ | ✓ | ✗ | ✓ |
...
**People who want to understand what their server is running.** General-purpose servers do the job, but they are large systems you configure and take on trust. Servette is one readable module (~4,400 lines of Python, no hidden machinery) that you can follow top to bottom in an afternoon.
...
| Path | What it is |
|---|---|
| `servette/` | The installable package: `__init__.py` is the entire product — server, system, and shell in one module, generated from `src/` and not edited by hand — beside a stub `__main__.py`. The error page is inlined into the module at build time from `src/404.html`, so an install is Python only |
