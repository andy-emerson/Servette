...
and `python -m build` call src/_literate_backend.py (the PEP 517 backend named
in pyproject.toml), which runs build() here and hands the result to setuptools,
so what ships is always the sources' build regardless of the committed copy.
--check holds the two equal, and the test suite regenerates before importing.
...
Usage:
    ...
    python build.py --check         # build in memory, diff against the
                                     # committed servette.py, exit 1 on drift
...
PUBLISH_SOURCE = "publish.html"
_PUBLISH_MARKER = "@@PUBLISH_HTML@@"
# Every page the module embeds: (source file, the marker it replaces). Each
# marker sits inside a triple-quoted literal in the sources.
EMBEDDED_PAGES = [(NOT_FOUND_SOURCE, _NOT_FOUND_MARKER),
                  (PUBLISH_SOURCE, _PUBLISH_MARKER)]
...
def main(argv=None):
    ...
    if args.check:
        ...
        diff = difflib.unified_diff(
            ...
            fromfile=f"{os.path.basename(target)} (committed)",
